...
const PDFViewerApplication = {
  ...
  async _initializeViewerComponents() {
    ...
    console.log("1 annotationEditorParams: ", appConfig.annotationEditorParams);
    if (appConfig.annotationEditorParams) {
      console.log("2 annotationEditorMode: ", annotationEditorMode);
      if (annotationEditorMode !== AnnotationEditorType.DISABLE) {
        const editorSignatureButton = appConfig.toolbar?.editorSignatureButton;
        console.log("3 editorSignatureButton: ", editorSignatureButton);
        if (editorSignatureButton && AppOptions.get("enableSignatureEditor")) {
          editorSignatureButton.parentElement.hidden = false;
        }
        this.annotationEditorParams = new AnnotationEditorParams(appConfig.annotationEditorParams, eventBus);
      } else {
        console.log("4 annotationEditorMode: ", annotationEditorMode);
...
document.addEventListener("mouseup", () => {
  ...
  if (selectedText) {
    const range = selection.getRangeAt(0);
    const rect = range.getBoundingClientRect();
    showCustomMenu(rect.left, rect.top, selectedText);
  }
});
